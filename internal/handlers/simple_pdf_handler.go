...
// SimplePDFHandler handles HTTP requests for simple PDF operations.
type SimplePDFHandler struct {
...
// NewSimplePDFHandler creates a new instance of SimplePDFHandler.
func NewSimplePDFHandler(simplePDFUseCase usecases.SimplePDFUseCase, logger *zap.Logger, numWorkers int, queueSize int) *SimplePDFHandler {
	// Create the worker pool with custom configuration.
	workerPool := NewPDFWorkerPoolWithQueueSize(numWorkers, queueSize, simplePDFUseCase, logger)
	workerPool.Start()

	// Create context for centralized monitoring.
	monitorCtx, monitorCancel := context.WithCancel(context.Background())

	handler := &SimplePDFHandler{
...
// CreateSimplePDF handles POST /pdf-simple/:id request
func (h *SimplePDFHandler) CreateSimplePDF(c *gin.Context) {
	...
	if !h.IsAvailable() {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "PDF service is not available"})
		return
	}
...
func (h *SimplePDFHandler) GetPoolStatus(c *gin.Context) {
	h.mu.RLock()
	workerPool := h.workerPool
	h.mu.RUnlock()
	...
	c.JSON(http.StatusOK, gin.H{
		"queue_size":     workerPool.GetQueueSize(),
		"active_workers": workerPool.GetActiveWorkers(),
		"total_workers":  workerPool.workers,
		"active_jobs":    workerPool.GetActiveJobs(),
...
func (h *SimplePDFHandler) IsAvailable() bool {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *SimplePDFHandler) StopWorkerPool() {
	h.mu.Lock()
	defer h.mu.Unlock()
