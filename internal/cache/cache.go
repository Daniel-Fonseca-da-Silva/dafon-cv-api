...
func (cs *CacheService) Set(ctx context.Context, key string, value interface{}, ttl time.Duration) error {
	...
	if err != nil {
		cs.logger.Error("Failed to marshal cache value",
			zap.Error(err),
			zap.String("key_type", getCacheKeyType(key)))
		return fmt.Errorf("failed to marshal cache value: %w", err)
	}

	err = cs.client.Set(ctx, key, jsonData, ttl).Err()
	if err != nil {
		cs.logger.Error("Failed to set cache value",
			zap.Error(err),
			zap.String("key_type", getCacheKeyType(key)))
		return fmt.Errorf("failed to set cache value: %w", err)
	}

	cs.logger.Debug("Cache value set successfully",
		zap.String("key_type", getCacheKeyType(key)),
...
func (cs *CacheService) Get(ctx context.Context, key string, dest interface{}) (bool, error) {
	...
	if err != nil {
		...
		cs.logger.Error("Failed to get cache value",
			zap.Error(err),
			zap.String("key_type", getCacheKeyType(key)))
	...
	if err != nil {
		cs.logger.Error("Failed to unmarshal cache value",
			zap.Error(err),
			zap.String("key_type", getCacheKeyType(key)))
...
func (cs *CacheService) Delete(ctx context.Context, key string) error {
	...
	if err != nil {
		cs.logger.Error("Failed to delete cache value",
			zap.Error(err),
			zap.String("key_type", getCacheKeyType(key)))
		return fmt.Errorf("failed to delete cache value: %w", err)
	}

	cs.logger.Debug("Cache value deleted successfully",
		zap.String("key_type", getCacheKeyType(key)))
	return nil
}
...
func (cs *CacheService) DeletePattern(ctx context.Context, pattern string) error {
	...
	if err != nil {
		cs.logger.Error("Failed to get keys by pattern",
			zap.Error(err),
			zap.String("pattern_type", getCacheKeyType(pattern)))
	...
	if err != nil {
		cs.logger.Error("Failed to delete keys by pattern",
			zap.Error(err),
			zap.String("pattern_type", getCacheKeyType(pattern)))
		return fmt.Errorf("failed to delete keys by pattern: %w", err)
	}

	cs.logger.Debug("Cache keys deleted by pattern",
		zap.String("pattern_type", getCacheKeyType(pattern)),
